...
<script lang="ts">
	...
	const fetchAndSetItems = async (isNavigate: boolean = false) => {
		...
		const resp = await fetch(
			`http://127.0.0.1:5173/api/user/${userId}/watchlist?` + params
		);
...
<div class="flex flex-row gap-x-10 grow mt-[-1rem]">
	<div class="flex flex-col w-full gap-y-4">
		<div class="flex flex-row w-full items-center gap-x-2 {(loading || $items.length > 0) ? '' : 'invisible'}">
